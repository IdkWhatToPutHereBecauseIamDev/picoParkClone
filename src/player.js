class PlayerHandler {
    ...
    updateControls() {
        ...
        for (let i = 0; i < players.length; i++) {
            this.updatePlayerControls(players[i])
        }
        updateControls()
    }
    ...
    updatePlayers() {
        ...
        for (let i = 0; i < players.length; i++) {
            ...
            if (player.ready) readyPlayers -= 1
        }
    }
    ...
    updatePlayerControls(player) {
        ...
        const leftPressed  = c.left.some(k => keys[k])
        const rightPressed = c.right.some(k => keys[k])
        const jumpPressed  = c.jump.some(k => keys[k])
...
class Player {
    constructor(game, options) {
        ...
        this.body = this.game.matter.addBody(
            v(30, (this.game.players.length * 50)),
            v(40, 46),
            {
                ...options.bodyOptions,
                inertia: Infinity,
            ...
            this.game.playerhandler.playerComp
        )
        ...
        this.hasShield = {1:false,2:false,3:false,4:false}
        this.preHasShield = {1:false,2:false,3:false,4:false}
        this.laserShields = []

        this.frame = "idle"
        this.preFalling = false

        this.groundDetector = Matter.Bodies.circle(
            this.body.position.x,
            this.body.position.y + (spriteSize.x * 0.5),
            spriteSize.x * 0.25,
            {
                collisionFilter: {
                    group: -1,
    ...
    giveShield(angle) {
        let axis = (angle % 2)
        let laserShield = Matter.Bodies.rectangle(
            this.body.position.x,
    ...
    testFalling() {
        if (this.body.position.y >= window.innerHeight * 3) {
            this.restart()
            Matter.Body.setPosition(this.body, v(100,-60))
        ...
        if (this.dead && !this.preDead) {
            this.body.collisionFilter = { mask:0, group:1, mask:-1 }
    ...
    onGround() {
        return Matter.Query.collides(
            this.groundDetector,
            Matter.Composite.allBodies(this.game.matter.engine.world)
                .filter(a => a.id != this.body.id && a.id != this.groundDetector.id && !a.shield)
        ).length > 0
    }
    ...
    testPlayerBlockCollision() {
        ...
        var ret = Matter.Query.collides(
            this.body,
            Matter.Composite.allBodies(this.game.matter.engine.world)
                .filter(a => a.id != this.body.id)
        )
    ...
    testPlayerCollision() {
        ...
        var ret = Matter.Query.collides(
            this.body,
            Matter.Composite.allBodies(this.game.matter.engine.world)
                .filter(a => a.id != this.body.id && ((a.isBlock) ? !a.block.locked() : true) && !a.shield)
        )
    ...
    updatePlayerParts() {
        ...
        Matter.Body.setPosition(
            this.groundDetector,
            v(this.body.position.x, this.body.position.y + (spriteSize.y * 0.5 * this.scale))
        )
        ...
        if (this.laserShields.length > 0) {
            for (let i = 0; i < this.laserShields.length; i++) {
                ...
                if (sh != undefined) {
                    Matter.Body.setPosition(
                        sh,
                        v(this.body.position.x + sh.laserShieldPos.x, this.body.position.y + sh.laserShieldPos.y)
                    )
